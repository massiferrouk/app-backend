...
public class AuthService {
    ...
    public AuthResponse refresh(RefreshRequest request) {
        ...
        if (!jwtUtil.isTokenValid(rawToken)) {
            throw new com.yuniv.backend.exception.UnauthorizedException("Invalid or expired refresh token");
        }

        // Vérifie que le token est bien en base et non révoqué
        String tokenHash = hashToken(rawToken);
        RefreshToken storedToken = refreshTokenRepository.findByTokenHash(tokenHash)
                .orElseThrow(() -> new com.yuniv.backend.exception.UnauthorizedException("Refresh token not found"));

        if (storedToken.getIsRevoked()) {
            throw new com.yuniv.backend.exception.UnauthorizedException("Refresh token has been revoked");
        }

        if (storedToken.getExpiresAt().isBefore(OffsetDateTime.now())) {
            throw new com.yuniv.backend.exception.UnauthorizedException("Refresh token has expired");
        }
